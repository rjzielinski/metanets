...
class fMRIDataset(torch.utils.data.Dataset):
    def __init__(
        self, img_dir, labels_file=None, transform=None, target_transform=None
    ):
        ...
        self.img_files = [
            ...
            if (os.path.isfile(os.path.join(img_dir, name)) and name.endswith(".gz"))
        ]
    ...
    def __getitem__(self, idx):
        ...
        if self.img_labels is not None:
            ...
            label = np.array([])
        if self.transform:
            image_norm = self.transform(image_norm)
        else:
...
train_loader = torch.utils.data.DataLoader(
    development_fmri, batch_size=batch_size, sampler=train_sampler
)
test_loader = torch.utils.data.DataLoader(
    development_fmri, batch_size=batch_size, sampler=test_sampler
)
